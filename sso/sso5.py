...
app.config['SAML_PATH'] = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'saml2')
...
def login_callback():
    ...
    if len(errors) == 0 and auth.is_authenticated():
        ...
        return render_template('index2.html')  # Redirect to home page
    else:
        return 'Login failed'
...
def index():
    if 'token' in session:
        print('已经有token')
        return render_template('index2.html') # Redirect to home page if token is available
    else:
        print('获取新token')
        req = prepare_flask_request(request)
        auth = init_saml_auth(req)
...
if __name__ == "__main__":
    app.run('127.0.0.1', debug=True, port=8200, ssl_context=('./aixint.cn_bundle.pem', './aixint.cn.key'))
